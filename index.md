...
![ecologytools logo](reference/figures/ecologytools.svg)

**Utilities and data-visualization tools for ecology in R**

[![License:
GPL-3](https://img.shields.io/badge/License-GPL--3-2A3B28?style=flat-square)](https://www.gnu.org/licenses/gpl-3.0)
[![R](https://img.shields.io/badge/R-%E2%89%A5%204.5.0-4E8F87?style=flat-square&logo=r&logoColor=white)](https://www.r-project.org/)
![Version](https://img.shields.io/badge/version-1.0-2A3B28?style=flat-square)![Status](https://img.shields.io/badge/status-active-40CFBB?style=flat-square)

[Installation](#install)  •  [FRESH](#fresh)  •  [Plotting](#plotting)
 •  [Spatial](#spatial)  •  [Forage & Diet](#diet)  • 
[Utilities](#utilities)

R utilities for ecology, centered on the
[`fresh()`](https://ryanmartinecology.github.io/ecologytools/reference/fresh.md)
linear-programming model of habitat-scale forage resources for
generalist herbivores and a suite of ggplot2 plotting tools — the
[`theme_martin()`](https://ryanmartinecology.github.io/ecologytools/reference/theme_martin.md)
publication theme with custom palettes and scales. Also includes
`terra`-based raster and terrain utilities, DBSCAN clustering of GPS
locations, and forage/diet-quality summaries.
...
**`fresh()` — maximize suitable forage via linear programming**

[`fresh()`](https://ryanmartinecology.github.io/ecologytools/reference/fresh.md)
computes the maximum suitable forage biomass that meets minimum
digestible energy (DE) and digestible protein (DP) concentration
constraints for a generalist herbivore, using linear programming. One LP
is solved per `patch_id`. It returns a `fresh_output` object with a
`$summary` (one row per patch x constraint pair), a `$detail` table
(per-forage contributions), and, when a `habitat_type` column is
supplied, a `$habitat` summary.
...
**Plot utility examples — `theme_martin()` & `scale_*_martin()`**
...
[`calc_diet_quality()`](https://ryanmartinecology.github.io/ecologytools/reference/calc_diet_quality.md)
ties the diet pipeline together: it weights each taxon’s stage-specific
forage quality by its RRA to produce per-sample DP and DE. It consumes a
phenology table from
[`calc_phenology_mode()`](https://ryanmartinecology.github.io/ecologytools/reference/calc_phenology_mode.md)
(`mode`), a stage-keyed quality table (`qual`, keyed by level name plus
`Phenology` and `Part`), and a taxonomy crosswalk (`lookup`).
