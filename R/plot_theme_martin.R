...
#' Built on `theme_void()` for predictable inheritance.
#'
...
theme_martin <- function(
  ...
  make_scale_layers <- function(values, aes, continuous) {
    if (continuous) {
      if (aes == "fill")  return(list(ggplot2::scale_fill_gradientn(colors = values)))
      if (aes == "color") return(list(ggplot2::scale_color_gradientn(colors = values)))
      return(list(
        ggplot2::scale_fill_gradientn(colors = values),
        ggplot2::scale_color_gradientn(colors = values)
      ))
    }
    if (aes == "fill")  return(list(ggplot2::scale_fill_manual(values = values)))
    if (aes == "color") return(list(ggplot2::scale_color_manual(values = values)))
    list(
